# Solar-Powered IoT Induction Cooker

## Nigerian Engineering Olympiad (NEO) - Energy & Sustainability Category

we're proposing a  smart cooking solution that combines induction heating technology with solar power and IoT capabilities to address Nigeria's energy challenges.
![final CAD model](https://github.com/Kosisochukwu244/Reeka/blob/main/inductionCooker_design1_ed.png)

## 🎯 Problem Statement

Nigeria faces significant challenges with:
- Unreliable electricity supply affecting daily cooking activities
- High cost of cooking fuel (gas) : on the average, nigerians spend up to ₦15,000 monthly on cooking gas - and that's ₦180,000 annually
- Environmental concerns from traditional biomass cooking
- Energy poverty in off-grid communities

Our solution provides a clean, efficient, and intelligent cooking alternative powered by renewable energy.

---

## 💡 Project Overview

This project develops an **induction cooker** that integrates:
- **Solar Power System**: Harnesses renewable energy for sustainable operation
- **IoT Integration**: Smart monitoring and control via Battery Management System (BMS) in addition to a mobile/web application 
- **Induction Heating Technology**: Efficient, safe, and fast cooking

### Key Features
- Solar panel integration for off-grid capability
- Real-time energy monitoring through IoT sensors
- Battery management system for energy storage optimization
- Remote monitoring and control capabilities
- Energy-efficient induction heating
- Safety features and auto-shutoff mechanisms

---

## 👥 Team Members

- Onyedikachi George - electrical/electronic design engineer
- Kosisochukwu Moronu - CAD design engineer/ embedded engineer

**Institution**: University of NIgeria Nsukka

---

## 🔧 Technical Components

### Hardware
- Induction heating coil and driver circuit
- Solar panels (specifications TBD)
- Battery storage system
- Battery Management System (BMS)
- Microcontroller/IoT module
- Power electronics and inverter
- Control interface
- (more to state as we progress)

### Software
- IoT firmware for monitoring and control
- BMS integration software
- Mobile/web dashboard (planned)
- Energy optimization algorithms

### Engineering Disciplines
- Electrical & Electronics Engineering
- Energy Systems
- Embedded Systems & IoT
- Power Electronics

---

![](https://github.com/Kosisochukwu244/Solar-Powered-IoT-induction-cooker/blob/main/full_design_again.png)
![first proposed CAD model](https://github.com/Kosisochukwu244/x/blob/ab3f9654700b4311f2e676b16e6d76bcb2f22140/inductionCooker_design1.png)


## 📊 Project Status

**Current Stage**: Concept Development & Design Phase

### Completed
- [ ] Problem identification and research
- [ ] Concept note submission
- [ ] Initial design concepts

### In Progress
- [ ] Detailed circuit design
- [ ] Solar system sizing and calculations
- [ ] BMS integration planning
- [ ] Component sourcing

### Upcoming
- [ ] Prototype development
- [ ] Testing and validation
- [ ] Performance optimization

---

## 🎯 Expected Impact

### Economic Benefits
- Reduced cooking costs for households
- Lower dependence on grid electricity
- Potential for local manufacturing

### Social Benefits
- Improved cooking safety
- Reliable cooking solution for off-grid communities
- Reduced indoor air pollution

### Environmental Benefits
- Zero emissions during operation
- Reduced carbon footprint
- Promotion of renewable energy adoption

---

## 📁 Repository Structure

```
├── docs/              # Documentation and design files
├── hardware/          # Circuit diagrams and hardware designs
├── software/          # Firmware and control software
├── simulations/       # Simulation files and results
├── prototype/         # Prototype development files
└── reports/           # Progress reports and presentations
```

---

## 🚀 Getting Started

*Documentation for setup and development will be added as the project progresses.*
![cad](https://github.com/Kosisochukwu244/x/blob/096318fb572b3cfa7352ba8f648b7f5988fe79c3/Screenshot%202025-12-12%20141801.png)
![cad2](https://github.com/Kosisochukwu244/x/blob/096318fb572b3cfa7352ba8f648b7f5988fe79c3/Screenshot%202025-12-12%20142007.png)


---

## 📞 Contact

For inquiries about this project:
- Email: kosisochukwu.moronu.244058@unn.edu.ng | Kachi.dev.004@gmail.com

---

## 📄 License & IP

This project is being developed for the Nigerian Engineering Olympiad (NEO). All intellectual property rights are retained by the team members as per NEO's IP Policy.

---

## 🙏 Acknowledgments

We acknowledge the support of:
- Nigerian Engineering Olympiad (NEO)
- Nigerian Content Development and Monitoring Board (NCDMB)
- Nigerian Society of Engineers (NSE)
- FIRST E&P
- Renaissance Africa Energy Company
- Enactus Nigeria
- University of Nigeria Nsukka
- Our mentors and advisors

---

**Last Updated**: December 2024

*This README will be updated regularly as the project progresses through the NEO competition stages.*
